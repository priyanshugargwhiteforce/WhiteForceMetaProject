...
const SingleAdAnalyzer = () => {
  ...
  const fetchAds = async (accountId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/meta/accounts/${accountId}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const fetchAdInsights = async (adId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/meta/ads/${adId}/insights`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
                    ...
                    <div className="flex items-center space-x-4">
                      <div className="flex flex-col items-center px-4 py-2 bg-slate-50 dark:bg-white/5 rounded-2xl border border-slate-100 dark:border-white/5">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-xs font-bold">{new Date(selectedAd?.created_time).toLocaleDateString()}</span>
                        </div>
                      </div>
                      <div className="flex flex-col items-center px-4 py-2 bg-slate-50 dark:bg-white/5 rounded-2xl border border-slate-100 dark:border-white/5">
