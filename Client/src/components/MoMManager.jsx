...
const MoMManager = () => {
  ...
  // Fetch Assignable Users for Task Assignment based on Role/Team Hierarchy
  const fetchAssignableUsers = async () => {
    try {
      const res = await axios.get('/api/moms/assignable-users', {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.data.success) {
      ...
      console.error('Failed to fetch assignable users for MoM:', err);
    }
  };
  ...
  const exportMoMToExcel = (item) => {
    ...
    if (Array.isArray(item.discussion_points) && item.discussion_points.length > 0) {
      pointsText = item.discussion_points.map((pt, idx) => {
        ...
        if (pt.assignee_name || pt.assign_to) {
          text += ` [ASSIGNED TASK TO: ${pt.assignee_name || `User #${pt.assign_to}`} | Priority: ${pt.priority || 'Medium'}]`;
        }
    ...
    const exportRows = momsList.map(item => {
      ...
      if (Array.isArray(item.discussion_points) && item.discussion_points.length > 0) {
        pointsText = item.discussion_points.map((pt, idx) => {
          let text = `Point ${idx + 1}: ${pt.topic}`;
          if (pt.assignee_name || pt.assign_to) {
            text += ` [Assigned: ${pt.assignee_name || `User #${pt.assign_to}`} (${pt.priority || 'Medium'})]`;
          }
          if (Array.isArray(pt.sub_points) && pt.sub_points.length > 0) {
                ...
                <div className="space-y-4">
                  {formPoints.map((pt, tIdx) => (
                    <div key={pt.id || tIdx} className="bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/10 rounded-xl p-4 space-y-3">
                      ...
                      {/* Action Task Assignment Bar */}
                      <div className="mt-3 pt-2.5 border-t border-slate-200/80 dark:border-white/10 flex flex-wrap items-center gap-3 bg-slate-100/70 dark:bg-slate-900/50 p-2.5 rounded-xl">
                        <div className="flex items-center space-x-1.5 text-xs font-bold text-slate-700 dark:text-slate-200 shrink-0">
                        ...
                        <div className="flex-1 min-w-[200px]">
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              const selectedUser = assignableUsers.find(u => Number(u.id) === Number(val));
                              handleTopicChange(tIdx, 'assign_to', val);
                              handleTopicChange(tIdx, 'assignee_name', selectedUser ? selectedUser.username : '');
                            }}
                            className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg px-2.5 py-1.5 text-xs font-semibold text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-blue-500"
                          >
                        ...
                        {pt.assign_to && (
                          <div className="flex items-center space-x-2 shrink-0">
                            <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400">Priority:</span>
                            <select
                              value={pt.priority || 'Medium'}
                              onChange={(e) => handleTopicChange(tIdx, 'priority', e.target.value)}
                              className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg px-2 py-1 text-xs font-bold text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-blue-500"
                            >
                              <option value="Low" className="bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-200 font-bold">Low</option>
                              <option value="Medium" className="bg-white dark:bg-slate-900 text-blue-600 dark:text-blue-400 font-bold">Medium</option>
                              <option value="High" className="bg-white dark:bg-slate-900 text-amber-600 dark:text-amber-400 font-bold">High</option>
                              <option value="Critical" className="bg-white dark:bg-slate-900 text-rose-600 dark:text-rose-400 font-bold">Critical</option>
                            </select>

                            <span className="text-[10px] font-extrabold px-2 py-0.5 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 rounded-md flex items-center space-x-1">
                              <Zap className="w-3 h-3 text-emerald-500" />
                              <span>Task will be assigned in Task Manager</span>
                            </span>
                          </div>
                        )}
                      </div>
                  ...
                  <div className="space-y-4">
                    {selectedMoM.discussion_points.map((pt, idx) => (
                      <div key={idx} className="bg-slate-50/70 dark:bg-white/[0.02] border border-slate-200 dark:border-white/10 rounded-xl p-4 space-y-2.5 shadow-sm">
                        ...
                        {(pt.assign_to || pt.assignee_name) && (
                          <div className="pt-2 mt-2 border-t border-slate-200/60 dark:border-white/5 flex items-center justify-between text-xs">
                            <span className="font-bold text-blue-600 dark:text-blue-400 flex items-center space-x-1.5">
                              <UserCheck className="w-3.5 h-3.5" />
                              <span>Assigned Action Task: <strong>{pt.assignee_name || `User #${pt.assign_to}`}</strong></span>
                            </span>
                            <span className="px-2 py-0.5 rounded text-[10px] font-extrabold bg-blue-500/10 text-blue-600 dark:text-blue-400 uppercase">
                  ...
                  <div className="space-y-1.5">
                    {selectedMoM.discussion_points.map((pt, idx) => (
                      <div key={idx} className="border border-slate-200 rounded-lg overflow-hidden bg-slate-50/40 mom-topic-card">
                        {/* Topic Title Header */}
                        <div className="bg-slate-100 border-b border-slate-200 px-2.5 py-1 flex items-center justify-between gap-2">
                          <h4 className="font-bold text-[11px] text-slate-900 flex items-center">
                            <span className="px-1.5 py-0.5 bg-rose-600 text-white rounded text-[8px] font-extrabold mr-1.5 uppercase tracking-wider shrink-0">
                              POINT {idx + 1}
                            </span>
                            <span>{pt.topic}</span>
                          </h4>
                          ...
                          {(pt.assign_to || pt.assignee_name) && (
                            <span className="px-1.5 py-0.5 bg-blue-100 text-blue-900 border border-blue-300 rounded text-[8px] font-black uppercase tracking-tight shrink-0 whitespace-nowrap">
                              ⚡ TASK ASSIGNED: {pt.assignee_name || `USER #${pt.assign_to}`} ({pt.priority || 'MEDIUM'})
                            </span>
                          )}
                        </div>
