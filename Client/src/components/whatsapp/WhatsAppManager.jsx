...
const WhatsAppManager = () => {
  ...
  const fetchWhatsAppDetails = async () => {
    try {
      setLoading(true);
      const response = await axios.get('http://localhost:5000/api/whatsapp/details');
  ...
  return (
    <div className="p-8 space-y-8">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        >
          <Activity className={`w-5 h-5 text-slate-500 ${loading ? 'animate-spin' : ''}`} />
        </button>
      </div>
...
const DetailItem = ({ icon: Icon, label, value }) => (
  <div className="p-4 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 flex items-center space-x-3 group/item hover:bg-white dark:hover:bg-white/10 transition-all">
    ...
    <div>
      ...
      <p className="text-sm font-bold truncate max-w-[150px]">{value || 'N/A'}</p>
    </div>
  </div>
);
