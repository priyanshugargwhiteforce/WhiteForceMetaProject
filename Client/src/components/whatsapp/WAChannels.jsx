...
const WAChannels = () => {
  ...
  const fetchChannels = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await axios.get('http://localhost:5000/api/whatsapp/channels', {
  ...
  const handleCreateChannel = async (e) => {
    ...
    try {
      ...
      const res = await axios.post('http://localhost:5000/api/whatsapp/channels', {
  ...
  const handleDeleteChannel = async (id, name) => {
    ...
    try {
      const res = await axios.delete(`http://localhost:5000/api/whatsapp/channels/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.data.success) {
        fetchChannels();
  ...
  const handleSaveUpdate = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`http://localhost:5000/api/whatsapp/channels/${updateChannelId}/updates`, {
  ...
  const fetchHistory = async (channelId) => {
    try {
      ...
      const res = await axios.get(`http://localhost:5000/api/whatsapp/channels/${channelId}/history`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.data.success) {
  ...
  const handleDeleteHistoryEntry = async (entryId) => {
    ...
    try {
      const res = await axios.delete(`http://localhost:5000/api/whatsapp/channels/updates/${entryId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.data.success) {
